...
Timing is absolutely crucial in this project. While optimizations are usually not the bread and butter of these kind of programming challenges, they are absolutely crucial in this case. The reason for this is that the timing of the bits is crucial to the correct operation of the system. If the timing is off, the system will not be able to read the data correctly, resulting in garbage data. The timing for correctly reading the data is as follows:
...
The timings above show that significant performance optimizations need to be implemented, to allow for the speeds that the system requires to correctly read the data. Some preemptive optimizations I can think of include:
...
While I did mention avoiding while loops above, this while loop has a very specific & niche function. It simply continuously polls the `micros()` function, until the time has elapsed. This is a very simple way of keeping track of the time, and is surprisingly accurate.
...
The transmitter has a much more simple state machine, as it only has to read the data from the serial port, and convert it into a bitstream.
...
The transmitter is responsible for reading the data from the serial port, and converting it into a bitstream. The bitstream is then sent to the receiver, which converts it into a byte.
...
In this section, I'll be covering the code around converting the retrieved data from the serial bus to the data that will be sent via GPIO. The code is split into several sections, each dealing with a specific part of the configuration.
...
For the data bits, it's fairly simple. We simply offset the data by the number of start bits, and then add the data bits. This is done in the following code:
...
For the parity bits, it's a bit more complicated, due to having to calculate the parity as well. Luckily, it's quite simple. We test to see if the current parity is equal to if we expect it to be even or odd, and add a 1 if it's not. Below is a table for additional explanation.
...
And last but not least, here is a Transmitter working at 9600 baud, with 1 start bit, 7 data bits, 1 stop bit, and no parity.
...
The receiver is responsible for reading the bitstream, and converting it into a byte. The byte is then sent to the serial port, where it can be read by the user.
...
The code for the receiver is significantly more difficult, as it has to read the data from the GPIO pins, and convert it into a byte. The code is split into several sections, each dealing with a specific part of the configuration.
...
Parity works very similar to the transmitter, though instead of inserting the parity into the data byte, to be sent over the protocol, it is instead used as a verification method, to test that the incoming data is correct. The following code is responsible for checking if the parity is correct:
...
While there is always room in the buffer, the buffer has been initialized to 0. This means, that in the event of a reset, the buffer will be filled with 0's. If the system doesn't read the correct number of stop bits, this check will fail.

#### Proof

![9600 Baud Proof](./Receiver_Pictures/9600_Proof.png)

This proved that my 8n1 configuration worked. Due to a lack of time, I haven't found the proper way to showcase that the various configurations work, but the explanation of the code above should be enough to showcase my knowledge of the subject.

## Conclusion

The project was a success, and I was able to program both the receiver and the transmitter to work together. The transmitter is able to send data at a baudrate of 9600, and the receiver is able to read the data at the same baudrate. The receiver is also able to read the data at a baudrate of 9600.
